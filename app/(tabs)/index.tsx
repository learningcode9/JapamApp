...
export default function JapamMain() {
  ...
  useEffect(() => {
    void (async () => {
      const [c, m, t] = await Promise.all([
        AsyncStorage.getItem(COUNT_KEY), AsyncStorage.getItem(MALAS_KEY), AsyncStorage.getItem(TOTAL_KEY),
      ]);
      setCount(Number(c ?? 0));
      setMalas(Number(m ?? 0));
  ...
  const saveSession = async (duration: number) => {
    ...
    const session: Session = { date: new Date().toISOString(), malas: 1, totalCount: 108, duration };
  ...
  const todayMalas = Math.floor(totalCount / 108);
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <Text style={styles.meta}>📿 {malas} malas   🔢 {totalCount} total   ⏱ {formatTime(seconds)}</Text>
      <Text style={styles.hint}>Tap resets at 108</Text>
      <Text style={styles.hint}>Today malas: {todayMalas} ({todayKey})</Text>

      <Pressable style={styles.circle} onPress={onTap} />

      <View style={styles.row}>
        <TextInput style={styles.input} value={minutesInput} onChangeText={setMinutesInput} keyboardType="numeric" />
        <Pressable style={styles.btn} onPress={() => setTargetSeconds((Number(minutesInput) || 1) * 60)}><Text style={styles.btnText}>Apply</Text></Pressable>
      </View>

      <View style={styles.row}>
        <Pressable style={styles.btn} onPress={() => { setSeconds(0); setIsRunning(true); }}><Text style={styles.btnText}>Start</Text></Pressable>
        <Pressable style={[styles.btn, styles.gray]} onPress={() => setIsRunning(false)}><Text style={styles.btnText}>Pause</Text></Pressable>
        <Pressable style={[styles.btn, styles.red]} onPress={() => { setIsRunning(false); setSeconds(0); setCount(0); }}><Text style={styles.btnText}>Reset</Text></Pressable>
      </View>
    </ScrollView>
  );
}
...
const styles = StyleSheet.create({
  ...
  circle: { width: 220, height: 220, borderRadius: 110, backgroundColor: '#6366f1', marginVertical: 24, shadowColor: '#000', shadowOpacity: 0.25, shadowRadius: 12, elevation: 6 },
  row: { flexDirection: 'row', gap: 10, marginTop: 12 },
  input: { backgroundColor: '#1e293b', color: 'white', borderRadius: 10, width: 90, textAlign: 'center', padding: 10 },
  btn: { backgroundColor: '#6366f1', paddingVertical: 10, paddingHorizontal: 18, borderRadius: 10 },
  gray: { backgroundColor: '#475569' }, red: { backgroundColor: '#ef4444' },
