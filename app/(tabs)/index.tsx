...
export const resolveHomeWorkspaceTotal = (
  candidate: number,
  workspaceId: string | null,
  totalsByWorkspace: Map<string | null, number>,
): number => {
  const previousTotal = totalsByWorkspace.get(workspaceId) ?? 0;
  const nextTotal = Math.max(candidate, previousTotal);
  totalsByWorkspace.set(workspaceId, nextTotal);
  return nextTotal;
};

export const isCurrentHomeWorkspaceRefresh = (
  request: HomeWorkspaceRefreshRequest,
  currentRequestGeneration: number,
  currentWorkspaceVersion: number,
  activeWorkspaceId: string | null,
): boolean => (
  currentRequestGeneration === request.generation
  && currentWorkspaceVersion === request.workspaceVersion
...
export const resolveLocalFirstTodayTotal = (
  ...
  const storedTodayTotal = options.storedTodayDate === todayKey
...
const HOME_SCOPED_MIGRATION_KEY = 'homeJapamScopedMigration';
...
const readHomeCounterSnapshot = async (
  userId: string | null,
  japamId: string | null,
): Promise<HomeCounterSnapshot> => {
  const scopedKeys = [
    getJapamScopedKey(TOTAL_KEY, userId, japamId),
    getJapamScopedKey(COUNT_KEY, userId, japamId),
    getJapamScopedKey(MALAS_KEY, userId, japamId),
    getJapamScopedKey(TOTAL_DATE_KEY, userId, japamId),
  ];
  const scoped = await AsyncStorage.multiGet(scopedKeys);
  const scopedValues = scoped.map(([, value]) => value);
  if (scopedValues.some((value) => value !== null)) {
    return {
      total: scopedValues[0] ?? null,
      count: scopedValues[1] ?? null,
      malas: scopedValues[2] ?? null,
      date: scopedValues[3] ?? null,
    };
  }

  if (!userId || !japamId) {
    return { total: null, count: null, malas: null, date: null };
  }

  const migrationKey = getUserStorageKey(HOME_SCOPED_MIGRATION_KEY, userId);
  if (await AsyncStorage.getItem(migrationKey)) {
    return { total: null, count: null, malas: null, date: null };
  }

  const legacyUser = await AsyncStorage.multiGet([
    getUserStorageKey(TOTAL_KEY, userId),
    getUserStorageKey(COUNT_KEY, userId),
    getUserStorageKey(MALAS_KEY, userId),
    getUserStorageKey(TOTAL_DATE_KEY, userId),
  ]);
  const legacyGlobal = await AsyncStorage.multiGet([TOTAL_KEY, COUNT_KEY, MALAS_KEY, TOTAL_DATE_KEY]);
  const userValues = legacyUser.map(([, value]) => value);
  const globalValues = legacyGlobal.map(([, value]) => value);
  const legacyValues = userValues.some((value) => value !== null) ? userValues : globalValues;

  const migrationWrites: [string, string][] = [];
  const scopedKeysToMigrate = [
    getJapamScopedKey(TOTAL_KEY, userId, japamId),
    getJapamScopedKey(COUNT_KEY, userId, japamId),
    getJapamScopedKey(MALAS_KEY, userId, japamId),
    getJapamScopedKey(TOTAL_DATE_KEY, userId, japamId),
  ];
  legacyValues.forEach((value, index) => {
    if (value !== null) migrationWrites.push([scopedKeysToMigrate[index], value]);
  });
  if (migrationWrites.length > 0) await AsyncStorage.multiSet(migrationWrites);
  await AsyncStorage.setItem(migrationKey, 'v1');

  return {
    total: legacyValues[0] ?? null,
    count: legacyValues[1] ?? null,
    malas: legacyValues[2] ?? null,
    date: legacyValues[3] ?? null,
  };
};
...
export default function JapamMain() {
  ...
  const activeWorkspaceIdRef = useRef<string | null>(currentJapamId);
  ...
  useEffect(() => {
    invalidateHomeWorkspace(currentJapamId);
  }, [currentJapamId, invalidateHomeWorkspace]);

  useEffect(() => {
    const subscription = DeviceEventEmitter.addListener('japam-did-switch', (payload?: { japamId?: string | null }) => {
      invalidateHomeWorkspace(payload?.japamId ?? null);
    });
    return () => subscription.remove();
  }, [invalidateHomeWorkspace]);
  ...
  const restoreTodayTotal = useCallback(async (options?: { skipRemoteFetch?: boolean }) => {
    ...
    if (restoringWorkspaceRef.current?.workspaceId === workspaceId) return;
    const request = {
      generation: ++restoreGenerationRef.current,
      workspaceId,
      workspaceVersion: workspaceVersionRef.current,
    };
    restoringWorkspaceRef.current = { workspaceId };
    const isCurrentRequest = () => isCurrentHomeWorkspaceRefresh(
      request,
    ...
    try {
      ...
      if (savedUserId) {
        const todayKey = getLocalDateKey();
        const storedCounters = await readHomeCounterSnapshot(savedUserId, workspaceId);
        const storedTotalDate = storedCounters.date;
        const storedTodayTotal = storedCounters.total;
  ...
  useEffect(() => {
    ...
    if (suppressCounterPersistenceRef.current) {
      suppressCounterPersistenceRef.current = false;
      return;
    }
