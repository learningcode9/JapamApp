...
describe('Groups dashboard cache-first and offline UX', () => {
  ...
  it('shows cached dashboard data offline on Android and reconciles after native reconnect', async () => {
    mockPlatformOS = 'android';
    mockIsNetworkFailure.mockImplementation((error: unknown) => error instanceof TypeError);
    mockGetCachedGroupDashboard.mockResolvedValue([row(UID, 'Offline Android Person')]);
    mockGetGroupDashboard
      .mockRejectedValueOnce(new TypeError('Network request failed'))
      .mockResolvedValue([row(UID, 'Reconciled Android Person')]);
    ...
    expect(allText(tree).join(' ')).toContain('Offline Android Person');
    expect(allText(tree).join(' ')).toContain("You're offline. Changes will sync when you're back online.");
    ...
    const texts = allText(tree).join(' ');
    expect(texts).toContain('Reconciled Android Person');
    expect(texts).not.toContain("You're offline. Changes will sync when you're back online.");
    expect(texts).not.toContain('Network request failed');
  });
  ...
  it('shows the no-cache message for a Failed to fetch error on Android', async () => {
    mockPlatformOS = 'android';
    mockIsNetworkFailure.mockImplementation((error: unknown) => error instanceof TypeError);
    mockGetGroupDashboard.mockRejectedValueOnce(new TypeError('Failed to fetch'));
