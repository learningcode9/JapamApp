...
describe('Home offline workspace total isolation', () => {
  ...
  it('mounts Home, switches A→B offline, and ignores a stale A refresh without unscoped writes', async () => {
    ...
    expect(await AsyncStorage.getItem('count:user-1:workspace-a')).toBe('1');
    expect(await AsyncStorage.getItem('malas:user-1:workspace-a')).toBe('2');
    expect(await AsyncStorage.getItem('totalCount:user-1:workspace-b')).toBeNull();
    expect(await AsyncStorage.getItem('totalCount')).toBe('217');
    expect(await AsyncStorage.getItem('count')).toBe('1');
    expect(await AsyncStorage.getItem('malas')).toBe('2');
    expect((AsyncStorage.setItem as jest.Mock).mock.calls.some(([key]) => key === 'totalCount')).toBe(false);
