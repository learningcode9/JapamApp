...
export const TIMER_PENDING_COMPLETION_LOOP_KEY = 'timerPendingCompletionLoop';
...
export function buildTimerPairs(snapshot: TimerStateSnapshot): [string, string][] {
  return [
    ...
    [TIMER_PENDING_COMPLETION_LOOP_KEY, snapshot.pendingCompletionLoop === '' ? '' : String(snapshot.pendingCompletionLoop ?? '')],
...
export interface RawJapamTimerState {
  ...
  sessionJapamId: string | null;
  sessionJapamName: string | null;
  pendingCompletionLoop: string | null;
}
...
export async function readJapamTimerState(
  ...
  return {
    ...
    sessionJapamId: entries[9]?.[1] ?? null,
    sessionJapamName: entries[10]?.[1] ?? null,
    pendingCompletionLoop: entries[11]?.[1] ?? null,
  };
}
