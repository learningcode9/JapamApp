...
const MALA_SUBJECT = '🪷 Every Mala Brings You Closer to Inner Peace';
const COUNT_SUBJECT = 'Your Japam Practice Is Building Quietly';

function isCountOnly(ctx: CampaignContext): boolean {
...
function buildCountHtml(ctx: CampaignContext): string {
  ...
  <tr><td style="padding:32px 32px 4px;">
    ...
    <p style="margin:14px 0 0;font-size:15px;line-height:1.75;color:${config.colors.textPrimary};">
      Over the last fifteen days, you've made space for your Japam practice.
      Even one recent practice is meaningful — a quiet practice can build steadily over time.
    </p>
  </td></tr>
  ...
  <tr><td style="padding:20px 32px 0;">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0"
      ...
      <tr>
        ...
        <td align="right" style="padding:14px 20px;font-size:18px;font-weight:700;color:${config.colors.primary};">${stats.totalSessions.toLocaleString()}</td>
      </tr>
      <tr style="background:${config.colors.background};">
        <td style="padding:14px 20px;font-size:14px;color:${config.colors.textPrimary};">✨ Days practiced</td>
        <td align="right" style="padding:14px 20px;font-size:18px;font-weight:700;color:${config.colors.primary};">${stats.daysPracticed.toLocaleString()}</td>
      </tr>
    </table>
  </td></tr>
...
function buildHtml(ctx: CampaignContext): string {
  return isCountOnly(ctx) ? buildCountHtml(ctx) : buildMalaHtml(ctx);
}

function buildText(ctx: CampaignContext): string {
  return isCountOnly(ctx) ? buildCountText(ctx) : buildMalaText(ctx);
}
